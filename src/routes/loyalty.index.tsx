...
export const Route = createFileRoute("/loyalty/")({
  component: LoyaltyLanding,
});

function LoyaltyLanding() {
  const { balance, state, resetDemo } = useLoyalty();
  return (
    <div>
      <div className="text-center">
        <p className="font-script text-3xl text-teal leading-none">earn as you go —</p>
        <h1 className="mt-1 font-display text-4xl">Trovin' Bytes</h1>
        <p className="mt-2 text-sm text-ink-soft">
          Get checked in at any booth. Stack Bytes. Cash them in for real stuff.
        </p>
      </div>
      ...
      <div className="mt-5 rounded-2xl border-2 border-line bg-paper p-5 shadow-brand-md">
        ...
        <p className="mt-1 font-display text-5xl text-navy">{balance} <span className="text-gold-600">B</span></p>
        <p className="mt-1 text-xs text-ink-soft">= ${(balance / 100).toFixed(2)} of reward value</p>
      </div>

      <div className="mt-5 grid grid-cols-2 gap-3">
        <Tile to="/loyalty/qr" icon={QrCode} title="My QR" desc="Show this at a booth" tint="bg-gold/20" />
        <Tile to="/loyalty/wallet" icon={Wallet} title="Wallet" desc="See your history" tint="bg-teal/20" />
        <Tile to="/loyalty/rewards" icon={Gift} title="Rewards" desc="Spend your Bytes" tint="bg-coral/20" />
        <Tile to="/loyalty/scan" icon={ScanLine} title="Scan" desc="Vendor side" tint="bg-navy/10" />
        <Tile to="/loyalty/vendor" icon={Store} title="Booth setup" desc="Tier + live status" tint="bg-paper" />
      </div>
      ...
      <section className="mt-6 rounded-2xl border border-line bg-paper p-5">
        ...
        <ul className="mt-3 space-y-2 text-sm text-ink-soft">
          <li className="flex gap-2"><Sparkles className="mt-0.5 h-4 w-4 shrink-0 text-gold" /> Check-in = <b>10 Bytes</b>. Vendors scan you — never the other way around.</li>
          ...
          <li className="flex gap-2"><Sparkles className="mt-0.5 h-4 w-4 shrink-0 text-navy" /> 1 Byte = $0.01 of reward value. Trovin' pays out, not vendors.</li>
        </ul>
      </section>

...
import { useLoyalty, ACTION_LABEL } from "@/hooks/useLoyalty";
import { ArrowDownLeft, ArrowUpRight } from "lucide-react";

export const Route = createFileRoute("/loyalty/wallet")({
  component: WalletPage,
});

function WalletPage() {
  const { balance, state } = useLoyalty();
  ...
  return (
    <div>
      <div className="rounded-2xl border-2 border-line bg-navy text-cream p-5 shadow-brand-lg">
        ...
        <p className="mt-1 font-display text-5xl">{balance} <span className="text-gold">B</span></p>
      ...
      <div className="mt-3 space-y-2">
        {state.ledger.length === 0 && (
          <div className="rounded-xl border border-dashed border-line bg-paper p-6 text-center text-sm text-ink-soft">
            No activity yet. Get scanned at a booth to earn your first Bytes.
          </div>
        )}
        {state.ledger.map((e) => {
          const positive = e.bytes > 0;
          return (
